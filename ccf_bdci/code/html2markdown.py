...
def html_table_to_markdown_new(html_table):
    soup = BeautifulSoup(html_table, 'html.parser')
    ...
    for table in tables:
        ...
        thead = table.find('thead')
        if thead:
            markdown_table += "| " + " | ".join([th.text.strip() for th in thead.find_all('th')]) + " |\n"
            markdown_table += "| " + " | ".join(["---" for _ in thead.find_all('th')]) + " |\n"

        for row in table.find_all('tr'):
            markdown_table += "| " + " | ".join([td.text.strip() for td in row.find_all('td')]) + " |\n"
